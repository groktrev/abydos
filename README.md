...
The `abydos` streaming search command provides access to the `abydos.distance`, `abydos.fingerprint`, `abydos.phonetic`, `abydos.stemmer`, and `abydos.tokenizer` modules using default class parameters and the `dist_abs()`, `fingerprint()`, `enocde_alpha()` or `encode()`, `stem()`, and `tokenize()` functions, respectively. See the Abydos [documentation](https://abydos.readthedocs.io/) for available algorithms.
## Examples
